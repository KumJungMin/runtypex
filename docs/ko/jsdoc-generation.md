...
기본 설정에서는 생성된 인터페이스가 매퍼 파일과 같은 폴더의
`runtypex.generated.ts` 파일에 기록됩니다. 같은 폴더에서 여러 매퍼가 같은
generated 파일명으로 resolve되면 하나의 파일로 합쳐집니다.
...
같은 폴더의 모든 매퍼 결과를 고정된 커스텀 파일 하나로 합치고 싶다면
...
출력 파일명이 원본 매퍼 파일명을 따르게 하려면 `generatedFileName`에 함수를
전달합니다.
...
문서는 resolved generated 파일명을 기준으로 그룹화됩니다. 두 매퍼 파일이 같은
generated 파일명으로 resolve되면 인터페이스가 하나의 파일로 합쳐집니다. 같은
파일 안에서 생성 인터페이스 이름이 충돌하면 기존처럼 에러가 발생합니다.

`generatedFileName`이 함수이면 runtypex는 기본적으로 `**/*.generated.ts`를
제외합니다. resolver가 다른 패턴의 파일을 만든다면 `exclude`를 명시적으로
전달하세요.
